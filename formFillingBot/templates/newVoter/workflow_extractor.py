...
def extract_workflow_components(form_workflow):
    ...
    prompts = {
        step: form_workflow[step].get("prompt", "")
...
def generate_workflow_py(workflow_steps, mandatory_documents, prompts, fields, validation_rules, output_file="workflow.py"):
    with open(output_file, 'w', encoding='utf-8') as file:
        ...
        file.write(f"form_workflow_steps = {json.dumps(workflow_steps, indent=4)}\n\n")
        
        file.write("# Mandatory Documents\n")
        file.write(f"form_mandatory_documents = {json.dumps(mandatory_documents, indent=4)}\n\n")
        
        file.write("# Workflow Prompts\n")
        file.write(f"form_prompts = {json.dumps(prompts, indent=4)}\n\n")
        
        file.write("# Workflow Fields\n")
        file.write(f"form_fields = {json.dumps(fields, indent=4)}\n\n")
        
        file.write("# Validation Rules\n")
        file.write(f"form_validation_rules = {json.dumps(validation_rules, indent=4)}\n\n")
